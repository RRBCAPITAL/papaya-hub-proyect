...
const ModalEdit = ({ idAnuncio, userId, nivel, setModalEditOpen, setIsEdited, anuncios, setAnuncios}) => {

    const id = idAnuncio;

    const niveles = ["NOTHING", "SIMPLE", "MOTOMAMI", "BICHOTA"];
    ...
    const updateProcess = async () => {
        const res = await axios.put(`/api/anuncio/${id}`, data).catch(error => (
          ...
          <form action="" onSubmit={handleSubmit} className='flex flex-col gap-2 justify-center items-center'>
            ...
            {niveles?.map((a) => (
              <div key={a} className="mb-2 flex justify-between w-full">
                  ...
                  {a === "NOTHING"? <label className="w-full font-bold p-1 border-b-4 border-red-500 mx-4 rounded">DESACTIVADO</label> : a === "SIMPLE" ? <label className="w-full mx-4 rounded text-center font-bold p-1 border-b-4 border-slate-600">SIMPLE</label> : a === "MOTOMAMI" ? <label className="text-center mx-4 rounded w-full font-bold p-1 border-b-4 border-blue-500">MOTOMAMI</label> : <label className="mx-4 rounded text-center w-full font-bold p-1 border-b-4 border-bor-red">BICHOTA</label>}
            ...
            <buttons className='flex gap-[10px] mb-5'>
                <button type='button' className='mt-10 mx-auto px-4 py-2 text-white rounded bg-red-800 hover:bg-red-900  hover:scale-110 transition-all ease-in-out duration-300' onClick={() => setModalEditOpen(false)}>Cancelar</button>
            <button type='submit' className='mt-10 mx-auto px-4 py-2 rounded text-white bg-violet-700 hover:bg-violet-800  hover:scale-110 transition-all ease-in-out duration-300'>Actualizar</button>
            </buttons>
          </form>
            <button  onClick={() => setModalEditOpen(false)} className='absolute top-0 right-0 hover:scale-125 transition-all ease-in-out duration-300 text-3xl text-red-500 font-extrabold rounded w-fit px-[10px] py-1'>
                x
            </button>
        </modal>
        </div>
