...
const ModalDeleteU = ({ idAnuncio, setModalDeleteOpen, setIsDeleted, userId}) => {

    const id = idAnuncio + userId;
      ...
      <div className='fixed flex items-center justify-center min-h-screen w-screen bg-[#000000e3]'>
        ...
        <modal className='relative flex flex-col items-center justify-center gap-4 rounded-[10px] w-fit h-fit bg-white px-20 py-4'>
          ...
          <form action="" onSubmit={handleSubmit} className='flex flex-col gap-2 justify-center items-center'>
            ...
            <button  onClick={() => setModalDeleteOpen(false)} className='absolute top-0 right-0 hover:scale-125 transition-all ease-in-out duration-300 text-3xl text-red-500 font-extrabold rounded w-fit px-[10px] py-1'>
                x
            </button>
        </modal>
        </div>
